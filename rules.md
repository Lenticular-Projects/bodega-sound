...
### 7. DOCUMENTATION PROTOCOL (The "Paper Trail")
- **DOCS FIRST**: No code is written until a corresponding design or plan exists in the `docs/` folder.
- **TIMESTAMPING**: All documentation updates must include a `YYYY-MM-DD` timestamp to track evolution.
- **CONTEXT AWARENESS**: Before starting any task, you must read the relevant files in `docs/` to understand the Swarm State.
- **HANDOFFS**: When completing a task, update `docs/progress_log.md` so the next agent knows the exact state of the union.
